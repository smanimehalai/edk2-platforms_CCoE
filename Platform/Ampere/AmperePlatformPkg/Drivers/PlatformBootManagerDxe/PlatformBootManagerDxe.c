  ...
  )
{
  ...
  *BootOptionsList = ReallocatePool (
                       ...
                       sizeof (EFI_BOOT_MANAGER_LOAD_OPTION) * (BootOptionNumber + 1),
  ...
  )
{
  ...
  Description = AllocateZeroPool ((ARRAY_SIZE (ControllerData.Mn) + 1) * sizeof (CHAR16));
  if (Description != NULL) {
    Char = Description;
  ...
  )
{
  ...
  DescMaxSize = StrSize (Manufacturer) + StrSize (Product) + 1;
  Description = AllocateZeroPool (DescMaxSize);
  if (Description == NULL) {
    DEBUG ((DEBUG_ERROR, "%a: %d: Out of Resource\n", __FUNCTION__, __LINE__));
    return NULL;
  }
  ...
  )
{
  ...
  do {
    ...
    DevicePathList1[2] = NextDevicePathNode (DevicePathList1[1]);
  ...
  do {
    ...
    DevicePathList2[2] = NextDevicePathNode (DevicePathList2[1]);
